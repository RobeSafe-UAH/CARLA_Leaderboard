...
class RobesafeAgent(AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        for parameter in parameters:
            if "t4ac/sensors" in parameter and "camera" in parameter: # and not "demo" in parameter:
    ...
    def sensors(self):
        """
        The position and orientation of these sensors are w.r.t. a left-handed frame (required by CARLA)!!
        Take into account that in the config file (launch) you specified them using right-handed (standard in robotics)
        """

        sensors = []

        for _, camera in enumerate(self.cameras_parameters):
            sensors.append({'type': 'sensor.camera.rgb', 
                            'x': camera['x'], 'y': -camera['y'], 'z': camera['z'], 
                            'roll': 0, 'pitch': 0, 'yaw': camera['yaw'], 
                            'width': camera['width'], 'height': camera['height'], 'fov': camera['fov'], 'id': camera['id']}) 
    ...
    def run_step(self, input_data, timestamp):
        ...
        if (self.trajectory_flag) and ("OpenDRIVE" in list(input_data.keys())):
            ...
            for signal in stop_signals:
                ...
                regelem.global_location.pose.position.y = signal['y']
                regelem.global_location.pose.position.z = signal['z']
                # print("Stop orientation: ", signal['orientation']) # TODO: Only + or - ?
                regulatory_elements.reg_elems.append(regelem)
    ...
    def control_callback(self, actual_speed):
        ...
        return control # Return to CARLA simulator
